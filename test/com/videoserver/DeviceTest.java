...
public class DeviceTest {
	...
	public static void main(String arg[])throws IOException, InterruptedException{
		DatagramSocket connector = new DatagramSocket(8978);

		DatagramPacket sendPacket = new DatagramPacket(COMMAND.getBytes(),COMMAND.length() ,InetAddress.getByName("127.0.0.1") , 8888);
		connector.send(sendPacket);

		byte[] recvBuf = new byte[50];
		DatagramPacket recvPacket = new DatagramPacket(recvBuf, recvBuf.length);

		connector.receive(recvPacket);

		String recvStr = new String(recvPacket.getData(),0,recvPacket.getLength());
		System.out.println(recvStr);
		if(! (recvStr.startsWith("OK"))){
			return;
		}

		connector.close();

		Integer port = Integer.parseInt(recvStr.substring(3));

		sendVideoToServer(port);

	}
	
	public static void sendVideoToServer(Integer port) throws IOException, InterruptedException{
