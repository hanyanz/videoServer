package com.videoserver.sendFile;
...
public class SendVideoFile{
	...
	public static String NULL_VIDEO = "找不到当天的视频信息源";
	public static String NO_MORE_VIDEO = "当前直接视频已播放完,你可以点击查看历史视频";
	
//	public static void main(String[] args){
//		try{
//			SendVideoFile sendFile = new SendVideoFile();
//			sendFile.sendVideoFile("127.0.0.1","0001");
//		}catch(Exception e){
...
//	}
	...
	public void sendVideoFile(String ip, String Imei) throws IOException{
		String fileName = "D:/videoFiles/" + Imei + "/" + DateUtil.getToday();
		try{
			InitSocket(ip);
			//ReadFromFile.readFileByBytes(fileName);
	        file = FileHelper.getFileToSend(new File(fileName));
	        if(file == null){
	        	Send(NULL_VIDEO.getBytes(),NULL_VIDEO.length());
	        }
	        in = new FileInputStream(file);
	        sendVideoFileCore();
	        while(isSend){
	        	file = FileHelper.getNextFileToSend(file);
	        	if( file == null){
	        		Send(NO_MORE_VIDEO.getBytes(),NO_MORE_VIDEO.length());
	        		break;
	        	}
	        	in = new FileInputStream(file);
	        	sendVideoFileCore();
	...
	public void sendHistoricalFile(String ip, File file) throws IOException, InterruptedException{
		InitSocket(ip);
		if(file == null){
			in.close();
		}
	...
	public void sendVideoFileCore() throws IOException, InterruptedException{
		 int seq_num = 0;
	     int bytes=0;
	...
	public  void InitSocket(String ip) throws UnknownHostException, SocketException{
